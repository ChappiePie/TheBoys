...
public class TBClientUtil {
    ...
    public static void renderHeatvision(PlayerRenderer renderer, MatrixStack matrix, IRenderTypeBuffer bufferIn, int packedLightIn, AbstractClientPlayerEntity player, boolean isRightEye, float red, float green, float blue) {
        ...
        GRClientUtil.renderFilledBox(matrix, bufferIn.getBuffer(GRClientUtil.GRRenderTypes.GRLIGHTNING), box.grow(0.0312D), 1F, 1F, 1F, 1f, packedLightIn);
        GRClientUtil.renderFilledBox(matrix, bufferIn.getBuffer(GRClientUtil.GRRenderTypes.GRLIGHTNING), box.grow(0.0625D), red, green, blue, 0.5F * 1f, packedLightIn);
        matrix.pop();
    }
