...
public class TBClientUtil {
    ...
    public static void renderHeatvision(PlayerRenderer renderer, MatrixStack matrix, IRenderTypeBuffer bufferIn, int packedLightIn, AbstractClientPlayerEntity player, boolean isRightEye, Color color) {
        ...
        HUClientUtil.renderFilledBox(matrix, bufferIn.getBuffer(HUClientUtil.HURenderTypes.LASER), box.inflate(0.0312D), 1F, 1F, 1F, 1f, packedLightIn);
        HUClientUtil.renderFilledBox(matrix, bufferIn.getBuffer(HUClientUtil.HURenderTypes.LASER), box.inflate(0.0625D), color.getRed() / 255, color.getGreen() /255, color.getBlue() /255, 0.5F * 1f, packedLightIn);
        matrix.popPose();
    }
}
