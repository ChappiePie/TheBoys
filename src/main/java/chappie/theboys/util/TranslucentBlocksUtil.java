...
public final class TranslucentBlocksUtil {

    public static final float NO_ALPHA = -1.0F;

    private TranslucentBlocksUtil() {
    }
    ...
    public static boolean canSeeThrough(BlockPos pos) {
        ...
        if (FMLEnvironment.dist == Dist.CLIENT && Minecraft.getInstance().getCameraEntity() != null) {
            try {
                ...
                for (XRayAbility xRayAbility : CommonUtil.listOfType(XRayAbility.class, CommonUtil.getAbilities(Minecraft.getInstance().getCameraEntity()))) {
                    if (xRayAbility.hitPos != null && xRayAbility.blockHitPos != null
                            && xRayAbility.translucentTimer.value(partialTick) != 0) {
    ...
    public static float resolveAlpha(BlockPos pos) {
        if (pos == null || FMLEnvironment.dist != Dist.CLIENT) {
            return NO_ALPHA;
        }
