...
public class SyringeRenderer extends GeoItemRenderer<SyringeItem> {
    ...
    @Override
    public void preRender(PoseStack poseStack, SyringeItem animatable, BakedGeoModel model, @Nullable MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int renderColor) {
        ...
        int color = -1;
        if (this.currentItemStack.getItem() instanceof SyringeItem) {
            color = SyringeItem.getColor(this.currentItemStack);
        }
