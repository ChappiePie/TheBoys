...
public class SyringeItem extends Item implements GeoItem {
    ...
    public ItemStack finishUsingItem(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity) {
        if (pLivingEntity instanceof Player player && !pLevel.isClientSide()) {
            ...
            if (player.getRandom().nextBoolean()) {
                var effects = ForgeRegistries.MOB_EFFECTS.getValues().stream().filter(p -> ForgeRegistries.MOB_EFFECTS.getKey(p).getNamespace().equals("minecraft")).toList();
                var mobEffect = effects.get(player.getRandom().nextInt(effects.size()));
                player.addEffect(new MobEffectInstance(mobEffect, 40, 3, false, true, true));
            }
        }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pHand) {
        ...
        if (cap != null && pHand == InteractionHand.MAIN_HAND) {
            if (pPlayer.getMainHandItem().getTag() != null && pPlayer.getMainHandItem().getTag().contains("vial")) {
                ...
                cap.triggerAnim("theboys_syringe_controller", true, "idk");
            }
        }
