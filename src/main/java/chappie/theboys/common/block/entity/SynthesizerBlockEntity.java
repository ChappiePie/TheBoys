            ...
            if (this.cookingProgress == this.cookingTotalTime) {
                ...
                if (stack.is(Items.POISONOUS_POTATO) || stack.getRarity() != Rarity.COMMON || stack.getItem() instanceof PotionItem) {
                    ...
                    if (level.random.nextInt(100) <= 25) {
                        resultItem = PotionUtils.setPotion(new ItemStack(Items.POTION), BuiltInRegistries.POTION.stream().filter(p -> BuiltInRegistries.POTION.getKey(p).getNamespace().equals("minecraft")).findAny().get());
                    }
                }
                stack.shrink(1);
                if (stack.isEmpty()) {
