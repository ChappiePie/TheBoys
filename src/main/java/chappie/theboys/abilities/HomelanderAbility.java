...
import xyz.heroesunited.generatorrex.network.SetTypeMessage;
...
public class HomelanderAbility extends TBAbility {
    ...
    public void onUpdate(PlayerEntity player) {
        ...
        if (player.world.isRemote && !AbilityHelper.keyPressed(1) && cap.getType() == 1) {
            Networking.INSTANCE.send(PacketDistributor.SERVER.noArg(), new SetTypeMessage(0));
        }

        GRPlayerUtil.sendStatusMessage(player, "" + tick, true);

        if (cap.getType() == 1) {
            AbilityHelper.Abilities.makeLaserLooking(player);
            tick++;
        } else if (cap.getType() == 2) {
            if (player.world.isRemote && Minecraft.getInstance().gameSettings.thirdPersonView <= 1) {
    ...
    public void setRotationAngles(PlayerEntity player, HUSetRotationAnglesEvent event) {
        ...
        if (cap.getType() == 2) {
            ...
            if (player.getPrimaryHand() == HandSide.RIGHT) {
                event.getPlayerModel().bipedRightArm.rotateAngleX = (float) Math.toRadians(-(45F + (MathHelper.cos(f))));
                event.getPlayerModel().bipedRightArm.rotateAngleZ = (float) Math.toRadians(-45F);
            }else{
                event.getPlayerModel().bipedLeftArm.rotateAngleX = (float) Math.toRadians(-(45F + (MathHelper.cos(f))));
        ...
        public static void livingFall(LivingFallEvent e) {
            if (e.getEntityLiving() instanceof PlayerEntity) {
                ...
                if (AbilityHelper.getEnabled(TBAbilityTypes.HOMELANDER, player) && e.getDistance() > 20) {
                    double xPos = player.getPosX() - 2.5 + player.world.rand.nextInt(5);
                    double yPos = player.getPosY() - 2.5 + player.world.rand.nextInt(5);
                    double zPos = player.getPosZ() - 2.5 + player.world.rand.nextInt(5);
